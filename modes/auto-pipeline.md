...
1. **Browser navigation (preferido):** La mayoría de portales de empleo (Lever, Ashby, Greenhouse, Workday) son SPAs. Navegar a la URL y tomar snapshot para renderizar y leer el JD.
   - Claude Code: `browser_navigate` + `browser_snapshot`
   - Copilot CLI: `chrome-devtools-navigate_page` + `chrome-devtools-take_snapshot`
2. **Web Fetch (fallback):** Para páginas estáticas (ZipRecruiter, WeLoveProduct, company career pages).
...
1. **Extraer preguntas del formulario**: Usar browser navigation para navegar al formulario y hacer snapshot. Si no se pueden extraer, usar las preguntas genéricas.
